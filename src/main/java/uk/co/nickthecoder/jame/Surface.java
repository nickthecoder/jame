 ...
 * surface, created using : {@link uk.co.nickthecoder.jame.Video#setMode(int, int)}. Your game's bitmaps
 * are loaded using : {@link #Surface(String)}.
 * <p>
 * The only drawing primitives currently implemented by Jame are : <code>setPixel</code>, <code>blit</code>,
 * <code>fill</code> and rendering of {@link uk.co.nickthecoder.jame.TrueTypeFont}s. There no methods to draw lines,
 * circles etc.
 * <p>
...
public final class Surface
{
    ...
    /**
     * 
     * @return The C pointer for the SDL_Surface structure. Used only by Jame itself.
     */
    long getPointer()
    {
        return pSurface;
     ...
     * vary. An easier to understand (but slower) version is {@link #getPixelRGBA(int, int)}.
     * 
     * @param x
     ...
     *            understand (but slower) version is {@link #setPixel(int, int, RGBA)}.
     */
    ...
    /**
     * This should only be called on the Surface returned from {@link Video#setMode}. Switches the back buffer, and the
     * visible buffer.
     * 
     * @throws JameRuntimeException
     */
     ...
     * The top left of <code>this</code> is lined up with <code>(x,y)</code> of <code>dest</code>. Uses
     * {@link BlendMode#NONE}.
     * 
     * @throws JameRuntimeException
     */
     ...
     * compatible, so it is highly recommended that you use convert on calls to
     ...
     * <p>
     * When the surfaces's rectangles miss each other, the algorithm is Order(1) (i.e. fast), but when the rectangles
     * overlap, it is O(n), where n is the area of overlap.
     * 
     * @param other
