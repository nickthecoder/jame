...
/**
 * Surface is the core class for all things related to video. The display is a surface, created
 * using : {@link uk.co.nickthecoder.jame.Video#setMode(int, int)}. Your game's bitmaps are loaded
 * using : {@link #Surface(String)}.
 * <p>
 * The only drawing primitives currently implemented by Jame are : <code>setPixel</code>,
 * <code>blit</code>, <code>fill</code> and rendering of
 * {@link uk.co.nickthecoder.jame.TrueTypeFont}s. There no methods to draw lines, circles etc.
 * <p>
...
public final class Surface
{
    ...
    /**
     * For internal use only. We need to create blank Surface objects, which can then be filled in
     * by the JNI calls such as surface_setMode.
     */
    Surface()
    {
    }
     ...
     * 
     * For efficient blitting, convert all loaded surfaces, so that they have the same format as the
     * display's Surface :
     * 
     * <code>
     * <pre>
     ...
     * @param height
     * @param alpha
     *        If false, then then each pixel only has red, green and blue values. If it is true,
     *        then the pixels will also have an alpha value, which determines how transparent the
     *        pixel is.
     ...
     *        pixel is.
     * @param hardware
     *        Whether the Surface should be stored on the GPU's memory, or the computer's main
     *        memory. If 'hardware' is false, then the Surface will be stored in the computer's main
     *        memory. If 'hardware' is true, then the surface <i>may</i> be stored on the GPU, but
    ...
    /**
     * Creates a copy of the Surface. The new Surface's pixel format will be compatable with the
     * display's Surface, so it might be different from the source surface's.
     * 
     * @throws JameRuntimeException
     */
    public Surface copy()
    ...
    /**
     * Frees the memory associated with this Surface. Any attempt to use this surface after it has
     * been free will throw an exception. Although free will be called when the Surface is garbage
     * collected, it is good practice to free the Surface as soon as possible.
     */
    public void free()
    {
    ...
    /**
     * Used on RGB surfaces (not RGBA surfaces) to set the alpha value used during blitting. This is
     * a per-surface value, i.e. the same alpha value is used for all pixels.
     * 
     * Internally, this calls SDL's SDL_SetAlpha( surface, 0, 255 ) when alpha is 255, and
     * SDL_SetAlpha( surface, SDL_SRCALPHA, alpha ) for all other values of alpha.
     * 
     * @param alpha
     ...
     * 
     * Internally, this calls SDL's setAlpha( 0, 255 ) when value is false, and setAlpha(
     * SDL_SRC_ALPHA, 255 ) when value is true.
     * 
     * @param value
     * @throws JameRuntimeException
    ...
    /**
     * Exposes SDL's setAlpha, but IMHO, it is clearer to use {@link #setPerSurfaceAlpha(int)} for
     * RGB surfaces, and {@link #setAlphaEnabled(boolean)} for RGBA surfaces.
     * 
     * @param srcAlpha
     * @param alpha
     */
    ...
    /**
     * Gets the value of a single pixel. Only works with 32 bit surfaces. The meaning of the result
     * will depend on the Surface's pixel format i.e. the order, and the number of bits for each
     * red/green/blue/alpha values will vary. An easier to understand (but slower) version is
     ...
     *        The y coordinate, the top edge is the surface is 0.
     * @param value
     *        The new value for the pixel. The meaning of this value will depend on the Surface's
     *        pixel format. For an easier to understand (but slower) version is
    ...
    /**
     * This should only be called on the Surface returned from {@link Video#setMode}. Switches the
     * back buffer, and the visible buffer.
     * 
     * @throws JameRuntimeException
     */
    void flip()
    ...
    /**
     * Blits <code>this</code> onto <code>dest</code>. The top left of srcRect is lined up with the
     * top left of destRect. Only the pixels within srcRect are blitted, and only the pixels in
     * destRect are altered. i.e. the clipping is defined by the area of overlap of the two
     * rectangles.
     * 
     * Uses {@link BlendMode#NONE}.
     */
    ...
    /**
     * Blits <code>this</code> onto <code>dest</code>. The top left of srcRect is lined up with
     * (x,y) of destRect. Only the pixels within srcRect are blitted.
     * 
     * Uses {@link BlendMode#NONE}.
     */
    ...
    /**
     * Converts a Surface so that it uses a pixel format compatible with the display surface.
     * Blitting is faster when the pixel formats are compatable, so it is highly recommended that
     * you use convert on calls to <code new {@link #Surface(String)}</code>.
     * 
    ...
    /**
     * Rotates and zooms a Surface, creating a new Surface. Due to the rotation, even if the scale
     * is <= 1, the resulting surface can be larger than the source.
     * 
     * @param angle
     ...
     * <p>
     * When the surfaces's rectangles miss each other, the algorithm is Order(1) (i.e. fast), but
     * when the rectangles overlap, it is O(n), where n is the area of overlap.
     * 
     * @param other
     *        The other Surface taking part in the pixel collision test.
     * @param dx
     *        The X offset for this surface. 0 would have the left edges of both surfaces lined up.
     *        A positive number: other is to the left of this.
     * @param dy
